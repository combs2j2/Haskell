...
millerRabin a p = 
	...
	-- if p is even and not equal to 2
		...
		if (member True testCongs) || (((powMod a (last (powers (p-1))) p) - 1) `mod` p == 0)
...
isProbablyPrime p n = do
			...
			let testBases = [(powMod p x (p-5)) + 2 | x <- [1..n]]
...
generateProbablePrimes (lo, hi) n =
	if hi < lo
		then error "hi value error: upper bound must be at least lower bound"
